...
use anyhow::Context;
use simplicity_amp_signer::{
    SignerNetwork, execute_audit_credentials, execute_native, export_audit_credentials_json,
};
use std::{
    fs::{self, OpenOptions},
    io::{Read, Write},
...
fn main() -> anyhow::Result<()> {
    ...
    anyhow::ensure!(
        args.len() >= 2,
        "usage: amp-audit new-wallet WALLET_FILE | OPERATION WALLET_FILE NETWORK < request.json"
    );
    ...
    let network = match args[2].as_str() {
        "elements-regtest" => SignerNetwork::ElementsRegtest,
        "liquid-testnet" => SignerNetwork::LiquidTestnet,
