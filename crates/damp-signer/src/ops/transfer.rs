...
pub fn sign_transfer(
    ...
    let recipient = receive::validate_recipient_address(
        network,
        &request.deployment,
    ...
    let fee_candidates = fee_candidates
        .into_iter()
        .filter(|u| u.opening().value > fee)
        .collect();
    ...
    let recipient_address = recipient.address().as_address();
    ...
    let mut value_blinded_outputs = vec![1];
    pset.add_output(Output::new_explicit(
    ...
    let mut recipients = vec![recipient.owner().public_key()];
    if regulated_change > 0 {
        let holder_script = protocol.user_script(sender.0)?;
        let (_, key) = derive_xprv(signer, crate::keys::KeyRole::Holder, sender.1)?;
        pset.add_output(Output::new_explicit(
            holder_script,
            regulated_change,
            regulated_asset,
    ...
    anyhow::ensure!(fee_change > 0, "transfer requires policy-asset change");
    ...
    pset.add_output(Output::new_explicit(
        ...
        fee_change,
        policy_asset,
        Some(BitcoinPublicKey::new(change.blinding_pubkey.context(
            "policy-asset change address is not confidential",
        )?)),
    ));
    ...
    let output_openings = blinding::blind_audited_values(
        &mut pset,
        &input_secrets,
    ...
    let verifier_witness = crate::covenant::audit_records::transfer_witness(
        &mut pset,
        &protocol,
        &anchor,
        verifier_witness,
    ...
    for (index, input) in all_inputs.iter().enumerate().take(first_fee_index).skip(1) {
        ...
        let keypair =
            Keypair::from_secret_key(elements::secp256k1_zkp::SECP256K1, &xprv.secret_key());
...
fn validate_holder_inputs(
...
) -> anyhow::Result<(elements::schnorr::XOnlyPublicKey, crate::keys::KeyIndex)> {
    ...
    for input in inputs {
        ...
        let (_, xprv) = derive_xprv(
            signer,
            crate::keys::KeyRole::Holder,
            locator.derivation_index,
        )?;
        let owner = xonly_from_xprv(&xprv);
        anyhow::ensure!(
            owner == locator.owner_public_key.public_key(),
    ...
    let (_, xprv) = derive_xprv(
        signer,
        crate::keys::KeyRole::Holder,
        first_locator.derivation_index,
    )?;
...
fn checked_sum(values: &[ValidatedUtxo]) -> anyhow::Result<u64> {
    values.iter().try_fold(0u64, |sum, value| {
        sum.checked_add(value.opening().value)
...
pub fn finish(
    ...
    let reviewed_fee = review.fee.get();
