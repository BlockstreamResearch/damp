...
/// Blind selected wallet-output values while leaving their asset IDs explicit.
/// Regulated transfers use the native audited blinding path below.
pub fn blind_values(
    pset: &mut PartiallySignedTransaction,
    input_secrets: &crate::blinding::secrets::InputOpenings<'_>,
...
/// Native confidential value-only blinding balances the G coefficient across assets.
...
pub fn blind_audited_values(
...
) -> anyhow::Result<crate::blinding::secrets::OutputOpenings> {
    anyhow::ensure!(
        indices.len() >= 2,
    ...
    for &index in indices {
        ...
        if o.asset == Some(regulated_asset) {
            damp_core::ledger::AuditAmount::try_from(o.amount.unwrap())?;
        }
    }
    ...
    let mut rng = thread_rng();
    let mut openings = crate::blinding::secrets::OutputOpenings::default();
    ...
    for &index in indices {
        let secret = openings
            .get(&index)
            .context("selected output opening is missing")?;
