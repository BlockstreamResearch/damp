//! Comparative four-linear-verify variant; public deterministic research fixtures only.
use hl::{
...
fn check(index:u32, parity:u1, root:u256,d:Point,tc:Point,td:Point,zv:u256,zb:u256,u:Point,v:Point){{
    ...
    assert!(match jet::eq_256(0,e) {{true=>false,false=>true,}});
    let ug:Ge=point_ge(u);let vg:Ge=point_ge(v);
    jet::linear_verify_1(((e,c),0),ug);
    jet::linear_verify_1(((e,dg),0),vg);
    let l:Ge=unwrap(jet::gej_normalize(jet::gej_ge_add((tcg,1),ug)));
    let m:Ge=unwrap(jet::gej_normalize(jet::gej_ge_add((tdg,1),vg)));
    jet::linear_verify_1(((zv,h),zb),l);
    jet::linear_verify_1(((zb,p),0),m);
}}
fn main(){{
...
"#,
    ...
    for i in 0..10 {
        s.push_str(&format!("check({},witness::C_PARITY_{i},witness::C_ROOT_{i},witness::D_{i},witness::TC_{i},witness::TD_{i},witness::ZV_{i},witness::ZB_{i},witness::U_{i},witness::V_{i});\n",i+1));
    }
    s.push_str("}\n");
    s
}
...
pub(crate) fn prove(
    ...
    let mut child = Command::new("python3")
        .arg("scripts/pgc-autonomous-linear-prover.py")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()?;
    child
...
fn main() -> anyhow::Result<()> {
    ...
    for field in [
        ...
        } else if field.starts_with('D') || field.starts_with('U') || field.starts_with('V') {
