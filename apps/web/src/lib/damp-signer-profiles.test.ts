...
vi.mock("../generated/damp-signer/simplicity_damp_signer", () => ({
  default: vi.fn(() => Promise.resolve()),
  DampSigner: class FakeDampSigner {
...
describe("in-memory signer profile sessions", () => {
  ...
  it("switches A to B to A directly and restores persisted debug profiles after reload", async () => {
    const signer = await import("./damp-signer");
    ...
    vi.resetModules();
    const reloaded = await import("./damp-signer");
    ...
    const debugStorage = localStorage.getItem("simplicity-damp:debug-signer-profiles:v1");
  ...
  it("fails closed when persisted debug material no longer matches its public profile identity", async () => {
    ...
    const key = "simplicity-damp:debug-signer-profiles:v1";
