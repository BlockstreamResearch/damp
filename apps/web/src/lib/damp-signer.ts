...
import type {
  DampSigner as WasmDampSigner,
} from "../generated/damp-signer/simplicity_damp_signer";
...
type SignerModule = typeof import("../generated/damp-signer/simplicity_damp_signer");
...
let signer: WasmDampSigner | undefined;
const signerSessions = new Map<string, WasmDampSigner>();
...
async function loadModule() {
  modulePromise ??= import("../generated/damp-signer/simplicity_damp_signer")
    .then(async (module) => {
...
async function createDebugSignerCandidate(mnemonic: string, network: SignerNetwork) {
  ...
  const candidate = new module.DampSigner(words, network);
...
export async function deriveDampKey(
  deploymentSalt: string,
  role: "holder" | "issuer",
  network?: SignerNetwork,
) {
  return requireSigner(network).deriveDampKey(deploymentSalt, role) as {
    sdk: string;
    derivationIndex: number;
    derivationPath: string;
    publicKey: string;
...
export async function inspectUtxos(utxos: Utxo[]) {
