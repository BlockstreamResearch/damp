...
describe("wallet state and persistence", () => {
  ...
  it("accepts confidential fee funding and prefers confirmed over pending outputs", async () => {
    const snapshot = await discover({
      dependencies: dependencies({
        scans: (value) => value.source === "wallet" && value.branch === 0 && value.index === 0
    ...
    expect(state(confidential)).toBe("ready");
    ...
    expect(state(confidential, { ...explicit, status: "unconfirmed" })).toBe("ready");
