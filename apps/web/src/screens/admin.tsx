...
import { RegtestProviderSettings } from "../components/regtest-provider-settings";
...
export function AdminSetup() {
  ...
  return (
    <AppShell eyebrow="Issuer console / Setup" title="Create or import a deployment">
          ...
          <Panel className="setup-main">
            ...
            <form className="form-stack" onSubmit={form.handleSubmit(preview)}>
              ...
              <div className="form-grid-two">
                ...
                <label><span className="field-label">Precision <span className="field-qualifier">0–8 decimals</span></span><input className="precision-input" aria-invalid={Boolean(form.formState.errors.precision)} aria-describedby={form.formState.errors.precision ? "setup-precision-error" : "setup-precision-help"} min={0} max={8} step={1} type="number" {...form.register("precision", { valueAsNumber: true })} />{form.formState.errors.precision ? <small id="setup-precision-error" className="field-error">{form.formState.errors.precision.message}</small> : <small id="setup-precision-help">Controls how asset amounts are displayed.</small>}</label>
                <label><span className="field-label">Initial supply <span className="field-qualifier">Display units</span></span><input aria-invalid={Boolean(form.formState.errors.supply)} aria-describedby={form.formState.errors.supply ? "setup-supply-error" : "setup-supply-help"} inputMode="numeric" {...form.register("supply")} />{form.formState.errors.supply ? <small id="setup-supply-error" className="field-error">{form.formState.errors.supply.message}</small> : <small id="setup-supply-help">Converted to exact base units using the selected precision.</small>}</label>
              </div>
      ...
      {mode === "import" && <div className="setup-layout"><Panel className="setup-main"><button className="setup-chooser-return text-button" type="button" onClick={() => setMode("choose")}><ArrowRight size={14} /> Back to setup choices</button><SectionHeading label="Public deployment import" title="Manifest JSON or HTTPS URL" /><details className="report-requirements"><summary>Regtest public provider</summary><RegtestProviderSettings /></details><div className="form-stack"><label>Public source<textarea ref={importFieldRef} aria-invalid={Boolean(importError)} aria-describedby={importError ? "setup-import-error" : "setup-import-help"} rows={12} value={importValue} onChange={(event) => { setImportValue(event.target.value); setImportError(undefined); }} />{importError ? <small id="setup-import-error" className="field-error">{importError}</small> : <small id="setup-import-help">Paste the issuer's public manifest JSON or URL to enable import. It must match the canonical registry default branch.</small>}</label><button className="button issuer-primary wide" disabled={Boolean(busyAction) || !importValue.trim()} type="button" onClick={importDeployment}><ShieldCheck size={16} /> {busyAction === "import" ? "Verifying canonical data…" : "Import public deployment"}</button>{importedDeployment && <div className="issuer-attachment"><div><strong>Public import complete</strong><small>{importedDeployment.issuerProfileId === signer.profileId && importedDeployment.issuerDerivationIndex !== undefined ? "Issuer control is attached to the active signer profile." : "No issuer authority is attached to the active signer profile."}</small></div>{(importedDeployment.issuerProfileId !== signer.profileId || importedDeployment.issuerDerivationIndex === undefined) && <button className="button secondary" disabled={Boolean(busyAction) || !signer.connected} type="button" onClick={() => void attachImportedIssuerControl()}>{busyAction === "attach" ? "Proving control…" : !signer.connected ? "Connect signer above to attach control" : "Attach issuer control"}</button>}</div>}</div>{message && <p className="inline-message" role="status" aria-live="polite">{message}</p>}</Panel><aside className="setup-aside"><SafetyNote title="Role-neutral import">Holder use needs only canonical public data. Governance and reissuance stay locked until the connected signer separately proves the issuer key.</SafetyNote></aside></div>}
    </AppShell>
  );
}
