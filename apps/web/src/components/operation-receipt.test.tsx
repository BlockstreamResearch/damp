...
describe("operation receipt panel", () => {
  it("renders a terminal txid receipt and deliberate reset action", () => {
    const onReset = vi.fn();
    render(<OperationReceiptPanel receipt={receipt} network="liquid-testnet" amountLabel="1.23 DAMP" resetLabel="Start a new transfer" tone="holder" onReset={onReset} />);
  ...
  it("uses issuer action grammar and omits a public explorer on regtest", () => {
    render(<OperationReceiptPanel receipt={{ ...receipt, operation: "reissuance" }} network="elements-regtest" amountLabel="9 DAMP base units" resetLabel="Start a new reissuance" tone="issuer" onReset={vi.fn()} />);
