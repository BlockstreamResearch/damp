...
\begin{document}
    ...
    \begin{abstract}
        We describe a confidential audit construction for regulated assets on Liquid.
    ...
    This paper's construction borrows the audit goal and twisted-ElGamal intuition of PGC \cite{pgc}, but it does not adopt PGC's account system or its equal-value proof.
    ...
    Appendix~\ref{app:code-math} gives a Simplicity implementation of the checks discussed in this paper and maps its numbered source listings to the mathematics.
    ...
    Write a Pedersen value commitment as $\operatorname{Com}_{H}(v,b)=vH+bG$.
    ...
    The verifier requires this explicit asset field and derives the unblinded generator $H_A$ from it.
    ...
    The regulated asset type remains public.
    ...
    \]
    Here the field constructors denote the required native encodings, not the jets' bit-level types.
    A missing output or an incorrect field variant causes rejection.
    The remaining transaction data come from the jets \cite{simplicity-jets}:
    \[
        \begin{aligned}
    ...
    Holder authorization and policy governance are assumptions of this audit construction.
    An exact-output policy is an enclosing-covenant rule that restricts future spending of listed outputs.
    For audit reporting, an offchain issuer process indexes a bounded chain interval from a configured provider, recovers amounts and reconciles the resulting state.
    Its report may be signed to authenticate the issuer's result. The proof below does not establish that the provider supplied a complete chain history.
    ...
    \[
        ...
        \right)\bmod q.
    \]
    The fixed domain-separation tag is \texttt{DAMP/audit/autonomous/v3}.
    ...
    The source benchmarks secrets in $[0,2^{64})$ against $G$, a range containing the native range-proof interval $[1,2^{63}]$ used here.
    ...
    A timeout leaves the amount unknown. These results do not guarantee complete recovery.
    ...
    TBD. Audit key rotation remains unresolved in this paper.

    \section{Security considerations}
    \label{sec:security}

    A successful holder spend under an enclosing covenant that enforces the stated coverage requirement has a native audit proof for each regulated output, tied to the transaction.
    ...
    The construction does not prove confidentiality for every environment, complete recovery against malicious senders, absence of inflation outside the scanned state, or freedom from protocol defects.
    ...
    The appendix gives the Simplicity checks and their mathematical correspondence.
    ...
    The assistants helped analyze source material, draft the paper, and review the Simplicity checks under the authors' direction.
