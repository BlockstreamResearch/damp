...
This appendix gives a SimplicityHL implementation of the checks discussed in this paper and maps the native audit verifier to Section~\ref{sec:construction}.
The audit module is \texttt{audit.simf}.
...
We write a compressed curve point as $(\eta,x_C)$, where $\eta$ is the SEC~1 parity bit defined in Section~\ref{sec:recovery-record}.
...
\begin{auditmapping}
  ...
  \auditmap{8}{Decompress the curve point selected by $(\eta,x)$. The \texttt{unwrap} operation rejects a non-point. This establishes a finite point on secp256k1.}
...
Let $t=\sha(\mathtt{DAMP/audit/v2})$.
...
The fixed role byte \texttt{00} is part of the transcript even though it is not a variable in $\Gamma$.
...
The substitution proof in Section~\ref{sec:proof-messages} explains why honest responses pass these exact operations.
...
The coverage argument also requires an output scan by the enclosing program.
Each confidential regulated output must contribute one to that scan. An explicit output must contribute its positive explicit value.
The scan must require a positive contribution and an owner-bound script for every regulated output.
...
Here depth four refers to the enclosing program's policy-proof structure.
The policy-proof checks belong to the enclosing program and are outside the audit relation developed here.
These line numbers refer to \texttt{verifier\_d4.simf}, not the shared audit module.
...
\begin{auditmapping}
  ...
  \auditmap{17}{Scan regulated inputs and count those that need policy proofs without decrypting their amounts.}
...
The sender constructs $R$ using ephemeral ECDH and authenticated encryption.
The issuer decrypts it with the audit key, then checks the recovered opening against both $C$ and $D$.
Neither operation appears in the SimplicityHL listing.
Likewise, bounded recovery computes Equation~\ref{eq:recovery-target} and searches offchain:
\[
  C-s^{-1}D
  =vH_A+bG-s^{-1}(bsG)
  =vH_A.
