...
for i,row in enumerate(data['outputs']):
    ...
    assert e!=0
    u=a.mul(e,c);vv=a.mul(e,d)
    assert a.add(tc,u) is not None and a.add(td,vv) is not None, 'fixture must retry nonce for nonidentity targets'
    result[f'U_{i}']=point(u);result[f'V_{i}']=point(vv)
